...
export const LICENSE_TOKEN_STORAGE_KEY = "slh_license_token";
...
export function getStoredLicenseState(now = new Date()): StoredLicenseState {
  if (typeof window === "undefined") return { key: "", instanceId: "", token: "", verifiedUntil: "", verified: false };
  ...
  const token = window.localStorage.getItem(LICENSE_TOKEN_STORAGE_KEY)?.trim() ?? "";
...
export function saveVerifiedLicense(key: string, verifiedUntil = new Date(Date.now() + ONE_DAY_MS).toISOString(), instanceId?: string, token?: string) {
  ...
  if (token) window.localStorage.setItem(LICENSE_TOKEN_STORAGE_KEY, token.trim());
...
export function useStoredLicense() {
  const [state, setState] = useState<StoredLicenseState>({ key: "", instanceId: "", token: "", verifiedUntil: "", verified: false });
